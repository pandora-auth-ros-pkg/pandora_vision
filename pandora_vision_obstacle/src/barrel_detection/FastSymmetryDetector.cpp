...
void FastSymmetryDetector::rotateEdges( vector<Point2f>& edges, int theta ) {
  ...
  for( int i = 0; i < edges.size(); i ++ ) {
    int rho = r2 * edges[i].x + r3 * edges[i].y + half_diag;
    *(reRows[rho]++) = r0 * edges[i].x + r1 * edges[i].y + fourth_rho;
...
void FastSymmetryDetector::vote( Mat& image, int min_pair_dist, int max_pair_dist ) {
  ...
  for( int i = 0; i < temp_edges.size(); i ++ )
    edges.push_back( Point2f( temp_edges[i].x - center.x, temp_edges[i].y - center.y ) );
...
pair<Point, Point> FastSymmetryDetector::getLine( float rho_index, float theta_index ) {
  ...
  pair<Point, Point> pairReturn(p0, p1);
  return pairReturn;

  //return pair<Point, Point> { p0, p1, };
}

void FastSymmetryDetector::getMaxDist( float* maxDist ) {
