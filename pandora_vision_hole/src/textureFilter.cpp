 ...
 *
 * File Description :
 *	Class for wall detection
 *	
 * Contents :	BGR2YCrCb
 * 				YCrCb2BGR
 * 				calculatePatternHistograms
 * 				calculatePointDistance
 * 				calculateTexture
 * 				calculateTextureArea
 * 				calculateWallHistogram
 * 				clusterPixels
 * 				comparePoints
 * 				createRegions
 * 				drawLabelsImage
 ...
 * 				findCenters
 * 				findPatternCenters
 * 				findPatternCenters2d_1
 * 				findPatternCenters2d_2
 * 				findPatternCenters3d_1
 * 				findPatternCenters3d_2
 * 				fragmentImage
 * 				fragmentNegativeImages
 * 				fragmentPositiveImages
 * 				getNumberOfBins
 * 				loadImages
 * 				loadPatterns
 * 				mergeCenters
 * 				printCenters
 * 				printPatternHistograms
 * 				readCentersFromFile
 * 				setColorSpace
 * 				setPath
 * 				showImages
 ...
 * @param str
 */
void TextureFilter::setPath(string str)
{	
	///Path
	path = str;
...
void TextureFilter::printPatternHistograms()
{
	for (int i=0; i<patterns.size(); i++)
	{
		...
		for (int k=0; k<pattern->m_hist.bins; k++)
			cout << pattern->m_hist.valuesNorm[k] << " ";
	}
}
 ...
 * @param color
 */
void TextureFilter::fragmentImage(IplImage* imgInput, int width, int height, string path, string id, CvScalar color)
{	
	int counter = 0;
	IplImage* img = NULL;
	if (this->colorSpace_ != BGR)
...
void TextureFilter::writeCentersToFile()
{
	...
	ofstream outfile(path.str().c_str());
...
void TextureFilter::readCentersFromFile()
{
	...
	{		
		while(true)
		{
			...
			{
				string str;
				getline(infile, str); //Read lines from file and create string array
				...
				if (this->dimension_ == 3)
				{
					...
					string str1 = str.substr(0,found1);
					string str2 = str.substr(found1+1,found2-found1-1);
					string str3 = str.substr(found2+1,str.length()-found2);
				...
				else
				{
					...
					string str1 = str.substr(0,found1);
					string str2 = str.substr(found1+1,str.length()-found1);
